...
% plot meanNMI rank
h = figure ('Visible', 'on', 'Position', [0,0,3,3], 'unit', 'inch');
set (gca, 'Position', [0.2, 0.15, 0.75, 0.75]);
disp(obj.result.meanRepro);
any(isnan(obj.result.meanRepro))
...
set (gca, 'Position', [0.2, 0.15, 0.75, 0.75]);
disp(1 - obj.result.beta_rank_subjLoad);
any(isnan(obj.result.beta_rank_subjLoad))
...
% test bar graphs
figure;
bar(obj.result.meanRepro, 'black');
xlabel('Component');
ylabel('Mean similarity');

figure;
bar(1 - obj.result.beta_rank_subjLoad, 'black');
xlabel('Component');
ylabel('Ratio of significant subjects');
...
% ----- My code
% For CIMT data
% atlas = load('/Users/fei/Desktop/Harris_Lab/dFNC/study_specific_atlas.mat'); 

% For FPI data
atlas = load('/Users/fei/Desktop/Harris_Lab/dFNC/FPI_RS_share/Template/FPI_study_specific_atlas.mat');
anat_2D_mosaic = raicar_toMosaic(flipdim(atlas, 2), 10);
% -----
...
for comp = 1:totalComp
    ...
    raicar_render (map2D, anat_2D_mosaic, 0);
